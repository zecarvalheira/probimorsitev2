  ...
  <?php foreach ($frames as $i => $frame): ?>
    <?php $line = $frame->getLine(); ?>
      <div class="frame-code <?php echo ($i == 0 ) ? 'active' : '' ?>" id="frame-code-<?php echo $i ?>">
        ...
        ?>
            <style>
             #frame-code-0 .linenums li:nth-child(<?php echo $line-$start ?>) {
                 background-color: rgba(255, 100, 100, .07); 
                 padding: 2px;
              }
             #frame-code-0 .linenums li:nth-child(<?php echo $line-$start+1 ?>) {
                 background-color: rgba(255, 100, 100, .17); 
                 padding: 2px;
              }
             #frame-code-0 .linenums li:nth-child(<?php echo $line-$start+2 ?>) {
                 background-color: rgba(255, 100, 100, .07); 
                 padding: 2px;
              }
            </style>
